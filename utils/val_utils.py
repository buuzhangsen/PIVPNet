...
def compute_psnr_ssim(recoverd, clean):
    ...
    for i in range(recoverd.shape[0]):
        ...
        current_recoverd = recoverd[i]

        # 处理单通道情况，压缩维度
        if current_clean.shape[-1] == 1:
            current_clean = current_clean.squeeze(axis=-1)
            current_recoverd = current_recoverd.squeeze(axis=-1)

        # 确定是否为多通道图像
        channel_axis = -1 if current_clean.ndim == 3 else None

        # 获取图像尺寸
        if current_clean.ndim == 3:
            h, w, _ = current_clean.shape
        else:
            h, w = current_clean.shape

        # 动态计算窗口大小，确保不超过图像尺寸且为奇数
        win_size = min(h, w, 7)
        if win_size % 2 == 0:
            win_size -= 1
        win_size = max(win_size, 3)  # 确保最小为3

        # 计算PSNR
        psnr += peak_signal_noise_ratio(current_clean, current_recoverd, data_range=1)
        # 计算SSIM
