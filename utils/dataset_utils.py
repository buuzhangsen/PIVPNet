...
class PIVPNetTrainDataset(Dataset):
    def __init__(self, args,mode='train'):
        super(PIVPNetTrainDataset, self).__init__()
        self.args = args
        self.rs_ids = []
        self.hazy_ids = []
    ...
    def _init_hazy_ids(self):
        ...
        self.hazy_ids = self.hazy_ids[:len(self.hazy_ids) * 0.8]
    ...
    def _init_deblur_ids(self):
        ...
        image_list = os.listdir(os.path.join(self.args.gopro_dir, 'input/'))
        temp_ids = image_list
        self.deblur_ids = [{"clean_id" : x,"de_type":5} for x in temp_ids]
        self.deblur_ids = self.deblur_ids * 10
    ...
    def _init_enhance_ids(self):
        ...
        self.enhance_ids = self.enhance_ids * 50
    ...
    def _get_gt_name(self, rainy_name):
        gt_name = rainy_name.split("rainy")[0] + 'high/norain-' + rainy_name.split('rain-')[-1]
        return gt_name


    def _get_deblur_name(self, deblur_name):
        gt_name = deblur_name.replace("input", "target")
        return gt_name
    

    def _get_enhance_name(self, enhance_name):
        gt_name = enhance_name.replace("low", "high")
        return gt_name

    def _get_nonhazy_name(self, hazy_name):
        nonhazy_name = hazy_name.replace("low", "high")
        name = hazy_name.split('/')[-1].split('_')[0]
        suffix = '.' + hazy_name.split('.')[-1]
        nonhazy_name = nonhazy_name.replace(hazy_name.split('/')[-1], name+suffix)
        return nonhazy_name
    # def _get_nonhazy_name(self, hazy_name):
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            if de_id == 3:
                ...
                degrad_img = crop_img(np.array(Image.open(os.path.join(self.args.gopro_dir, 'input/', sample["clean_id"])).convert('RGB')), base=16)
                clean_img = crop_img(np.array(Image.open(os.path.join(self.args.gopro_dir, 'target/', sample["clean_id"])).convert('RGB')), base=16)
...
class DerainDehazeDataset(Dataset):
    ...
    def _init_input_ids(self):
        if self.task_idx == 0:
            self.ids = []
            name_list = os.listdir(self.args.derain_path + 'low/')
            self.ids += [self.args.derain_path + 'low/' + id_ for id_ in name_list]
        elif self.task_idx == 1:
            self.ids = []
            name_list = os.listdir(self.args.dehaze_path + 'low/')
            self.ids += [self.args.dehaze_path + 'low/' + id_ for id_ in name_list]
...
class DerainDehazeDatasetval(Dataset):
    def __init__(self, args, task="enhance",addnoise = False,sigma = None):
        super(DerainDehazeDatasetval, self).__init__()
        self.ids = []
        self.task_idx = 0
        self.args = args

        self.task_dict = {'derain': 0, 'enhance': 3}
        self.toTensor = ToTensor()
        self.addnoise = addnoise
        self.sigma = sigma
    ...
    def __getitem__(self, idx):
        ...
        return [degraded_name,degrad_type], degraded_img, clean_img
